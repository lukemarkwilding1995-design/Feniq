...
| 5   | New Inspection                                    | Demo workflow: details → checks → diagnosis → repair/report; explicit private server draft/resume | Autosave, offline handling and multi-device conflict UX beyond optimistic version checks                                 |
...
| 20  | Professional PDF reports                          | Demo workflow: tenant-branded authenticated live PDF, append-only exact-byte revisions, source/PDF checksums and authenticated download history | Controlled external delivery and long-term object-storage policy                              |
...
| 30 | Product Passports | Demo workflow: persistent Site/Product IDs, company catalogue, retained lifecycle notes and inspection links, versioned site and product identity correction, and controlled archive/restore with immutable history and operational blocking | Controlled site/customer reassignment and inspection-link correction, verified manufacturer identity, richer lifecycle events, external sharing and production governance |
...
Migration 0011 adds a company-admin customer request register for access, correction and deletion enquiries, with retained status events and optimistic updates. The UI tracks intake and decisions but performs no automatic disclosure, correction or erasure. This is distinct from Technical Cases because requests need not have a linked inspection or Product Passport. See `PRIVACY_REQUEST_REGISTER_CYCLE_2026-09-22.md`.

Admin-only privacy requests now have a checksum-bound inventory of explicitly linked customer, site, passport, work-order, inspection and technical-case records. A retained review records manual identity and scope confirmations; closing is blocked if linked records change or an intervening status change makes the review stale. Active Access requests with a current review can show an internal, no-store draft of linked records for admin inspection. This is a review gate and draft, not an automated disclosure/erasure fulfilment mechanism. See `REQUEST_SCOPE_REVIEW_CYCLE_2026-09-22.md`.
...
Migration 0016 adds append-only exact PDF report revisions. Each retained artifact stores its original bytes, PDF checksum and a canonical checksum of the rendered source state, including photo bytes, citations, outcome and customer acceptance. Later changes mark prior revisions historical without altering their downloads. Privacy inventory metadata includes acceptance and report-revision history. See `IMMUTABLE_REPORT_REVISIONS_CYCLE_2026-09-25.md`.

Authenticated downloads of current reports and exact retained revisions now create company-scoped audit events with actor, timestamp, report kind, retained revision where applicable and PDF checksum. Assigned engineers and company administrators can review the report access history from the inspection screen. No migration is required. See `REPORT_ACCESS_AUDIT_CYCLE_2026-09-25.md`.

Migration 0017 adds admin-controlled PDF report name, business contact line and validated accent colour. The effective branding participates in the canonical report source checksum, so a change marks prior retained PDFs historical without altering their bytes. FenIQ attribution and all evidence/approval warnings remain visible. This is the first bounded Module 51 workflow, not a complete white-label system. See `REPORT_BRANDING_CYCLE_2026-09-25.md`.
