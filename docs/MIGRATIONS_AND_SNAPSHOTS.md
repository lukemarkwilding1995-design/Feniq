...
Inspection drafts are now durable through revision `0014_inspection_drafts`. They are explicit saves, private to the signed-in engineer within their company, version checked, and removed atomically when the matching inspection is completed. Drafts exclude engineer approval and customer sign-off; resuming the result step reruns the current diagnostic rules.

Still needed: diagnostic-run revision history, automatic offline recovery, immutable photo/review/outcome event references, richer source provenance, production retention controls and a full tenant-policy audit. Snapshot immutability does not authorise any remedial or chargeable action.
